...
EOF
    ...
    /usr/bin/mariadbd --user=mysql --bootstrap --verbose=0 --skip-name-resolve --bind-address=0.0.0.0 < "$tfile"
    rm -f "$tfile"
    ...
    if [ "$MYSQL_DATABASE" != "" ] && [ "$(ls -A /docker-entrypoint-initdb.d 2>/dev/null)" ]; then
        ...
        /usr/bin/mariadbd --user=mysql --skip-name-resolve --bind-address=0.0.0.0 --silent-startup > "${TEMP_OUTPUT_LOG}" 2>&1 &
        PID="$!"
    ...
    echo "exec /usr/bin/mariadbd --user=mysql --console --skip-name-resolve --bind-address=0.0.0.0" "$@"
fi
